...
function comment_tracker_entity_menu($hook, $type, $return, $params) {
	...
	if (in_array($entity->getSubtype(), $subscription_subtypes)) {
		...
		// TODO Get the popup though an XHR request
		...
		$return[] = ElggMenuItem::factory(array(
			'name' => 'comment_tracker_settings',
			'text' => elgg_view_icon('hover-menu'), // TODO Decide from hover-menu, settings, settings-alt
			'href' => "#comment-tracker-popup-{$entity->guid}",
			'rel' => 'popup',
			'priority' => 150,
			'title' => elgg_echo('comment_tracker:subscribtion_settings:tooltip'),
		));
...
function comment_tracker_savesettings($hook, $type, $return, $params) {
	...
	foreach ($notification_handlers as $method) {
		...
		/*
		TODO Create an upgrade script that converts these relationships to plugin usersettings
